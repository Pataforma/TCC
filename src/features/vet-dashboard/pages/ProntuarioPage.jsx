  ...
  useEffect(() => {
    const carregarDados = async () => {
      try {
        ...
        // Carregar paciente via API
        const pacienteData = await api.get(`/pacientes/${id}`);
        if (pacienteData) {
          setPaciente({
            ...pacienteData,
            tutor: pacienteData.tutor_nome || 'N/A',
            telefone: pacienteData.tutor_telefone || 'N/A',
            email: pacienteData.tutor_email || 'N/A',
        ...
        try {
          const prontuariosData = await api.get(`/prontuarios?paciente_id=${id}`);
          if (Array.isArray(prontuariosData) && prontuariosData.length > 0) {
            // Usar o primeiro prontuário ou o mais recente
            const prontuario = prontuariosData[0];
            
            // Carregar prescrições do prontuário
            try {
              const prescricoesData = await api.get(`/prescricoes?prontuario_id=${prontuario.id}`);
            ...
            // Carregar exames do prontuário
            try {
              const examesData = await api.get(`/exames?prontuario_id=${prontuario.id}`);
          ...
          console.log('Nenhum prontuário encontrado para este paciente');
        }

        // Carregar histórico clínico a partir de consultas
        try {
          const consultasData = await api.get(`/consultas?paciente_id=${id}`);
          if (Array.isArray(consultasData)) {
            setHistoricoClinico(
              consultasData.map((c) => ({
                id: c.id,
        ...
        // Carregar produtos disponíveis no estoque
        try {
